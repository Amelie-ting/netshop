...
<body>
	<c:choose>
		<c:when test="${empty cartItemList }">还木有订单哦~ 快去给心仪的宝贝下单吧</c:when>
		<c:otherwise>
			<form id="form1" action="<c:url value='/OrderServlet'/>"
				...
				<table width="95%" align="center" cellpadding="0" cellspacing="0">
					...
					<tr>
						<td><a
							href="<c:url value='/AddressServlet?method=loadaddress'/>">管理收货地址</a>
						</td>
					</tr>

					<tr bgcolor="#efeae5">
						<td width="400px" colspan="5"><span style="font-weight: 900;">我的订单</span></td>
					</tr>
					<tr align="center">
						<td width="10%">&nbsp;</td>
						<td width="50%">商品名称</td>
					...
					<c:forEach items="${cartItemList }" var="criteria">
						<tr align="center">
							<td align="right"><a class="linkImage"
								href="<c:url value='/jsps/book/desc.jsp'/>"><img border="0"
									width="54" align="top"
									src="<c:url value='/${criteria.items.item_pic }'/>" /></a></td>
							<td align="left"><a
								href="<c:url value='/jsps/book/desc.jsp'/>"><span>${criteria.items.item_name }</span></a>
							</td>
							<td>&yen;${criteria.items.item_price }</td>
							<td>${criteria.quantity }</td>
							<td><span class="price_n">&yen;<span class="subtotal">${criteria.subtotal }</span></span>
							</td>
						</tr>
					</c:forEach>
					<tr>
