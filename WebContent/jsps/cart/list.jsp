						...
						function() {
							...
							if (quantity == 1) {
								if (confirm("您是否真要删除该条目？")) {
									location = "/netstore/CartItemServlet?method=batchDelete&cartItemIds="
											+ id;
								}
							} else {
				...
				<c:forEach items="${cartItemList }" var="cartItem">
					<tr align="center">
						...
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/itemServlet?method=load&bid=${cartItem.items.item_id }'/>"><img border="0"
								width="54" align="top"
								src="<c:url value='/${cartItem.items.item_pic }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/itemServlet?method=load&bid=${cartItem.items.item_id }'/>"><span>${cartItem.items.item_name }</span></a>
						</td>
						<td><span>&yen;<span class="currPrice">${cartItem.items.item_price }</span></span></td>
