...
public class CartItemServlet extends BaseServlet {
	...
	public String add(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 * Map map = req.getParameterMap(); CartItem cartitem =
		 * CommonUtils.toBean(map, CartItem.class); Items item =
		 * CommonUtils.toBean(map, Items.class); User user = (User)
		 * req.getSession().getAttribute("usersession");
		 * cartitem.setItems(item); cartitem.setUser(user);
		 */

		/*
		 * 这里按照项目里提供的方法注入是不行的，还是需要通过getparameter来获得参数 然后再将参数添加进去
